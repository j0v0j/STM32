...
void CAN1_Configuration()
{
  ...
  NVIC_Init(&nvic);
	...
	can.CAN_Prescaler = 8;     //CAN BaudRate 36/(1+5+3)/4=1Mbps
	CAN_Init(CAN1, &can);
...
void CAN_DRV_Velocity_Position_Mode(unsigned char Group,unsigned char Number,short Temp_PWM,short Temp_Velocity,long Temp_Position)
{
    ...
    if(Temp_Velocity < 0)
    {
        Temp_Velocity = abx(Temp_Velocity);
    }
...
*****************************************************************************************/
void CAN_DRV_Config(unsigned char Group,unsigned char Number,unsigned char Temp_Time1,unsigned char Temp_Time2)
{
...
long Real_Position_Value[4] = {88,77,66,55};
...
void USB_LP_CAN1_RX0_IRQHandler(void) //CAN RX
{
    ...
    if (CAN_GetITStatus(CAN1,CAN_IT_FMP0)!= RESET)
	{
        ...
        if((rx_message.IDE == CAN_Id_Standard)&&(rx_message.IDE == CAN_RTR_Data)&&(rx_message.DLC == 8)) //标准帧、数据帧、数据长度为8
        {
            ...
            else if(rx_message.StdId == 0x02B)
            {
