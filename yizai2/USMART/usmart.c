...
void usmart_reset_runtime(void)
{
	...
	TIM_SetAutoreload(TIM6,0XFFFF);//将重装载值设置到最大
...
{
	if(TIM_GetFlagStatus(TIM6,TIM_FLAG_Update)==SET)//在运行期间,产生了定时器溢出
	{
		usmart_dev.runtime+=0XFFFF;
	}
	usmart_dev.runtime+=TIM_GetCounter(TIM6);
	return usmart_dev.runtime;		//返回计数值
}
//下面这两个函数,非USMART函数,放到这里,仅仅方便移植. 
//定时器4中断服务程序	 

//将usmart扫描函数放到FreeRTOS创建的函数中，这样可以使FreeRTOS去管理USMART的函数防止了错误
/////////////////////////////////////////////////////Error:..\FreeRTOS\portable\RVDS\ARM_CM3\port.c,378
//完成上面操作后，可将TIM6中断函数去掉
...
//		usmart_dev.scan();	//执行usmart扫描	
		TIM_SetCounter(TIM6,0);		//清空定时器的CNT
		TIM_SetAutoreload(TIM6,100);//恢复原来的设置		    				   				     	    	
	}				   
...
void Timer6_Init(u16 arr,u16 psc)
{
  TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
	...
	TIM_ITConfig( TIM6, TIM_IT_Update|TIM_IT_Trigger, ENABLE );//TIM4 允许更新，触发中断
...
{
#if USMART_ENTIMX_SCAN==1
	Timer6_Init(1000,(u32)sysclk*100-1);//分频,时钟为10K ,100ms中断一次,注意,计数频率必须为10Khz,以和runtime单位(0.1ms)同步.
#endif
