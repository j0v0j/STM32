...
//		HAL_TIM_Base_Stop_IT(&TIM2_Handler); //使能定时器2和定时器2更新中断：TIM_IT_UPDATE    
...
{  
    ...
    TIM7_Handler.Init.Period=(1000000/arr) - 1;                        //自动装载值
...
void Timer3_disable(void)					   //TIM3失能
{
	__HAL_RCC_TIM3_CLK_DISABLE();
	
	HAL_NVIC_DisableIRQ(TIM3_IRQn);
}
void Timer3_enable(void)					   //TIM3失能
{
...
void Timer7_disable(void)					   //TIM3失能
{
	__HAL_RCC_TIM7_CLK_DISABLE();	
	HAL_NVIC_DisableIRQ(TIM7_IRQn);
}

void Timer7_enable(void)					   //TIM3失能
{
...
void HAL_TIM_Base_MspInit(TIM_HandleTypeDef *htim)
{
			...
			HAL_NVIC_SetPriority(TIM7_IRQn,1,0);    //设置中断优先级，抢占优先级1，子优先级3
