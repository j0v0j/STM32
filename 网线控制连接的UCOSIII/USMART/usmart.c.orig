	...
	{					   
		...
		case 2://查询指令
			...
			for(i=0;i<usmart_dev.fnum;i++)
			{
				...
				Eth_send((u8 *)usmart_dev.funs[i].name);
				Eth_send((u8 *)"\r\n");
			}
			printf("\r\n");
			break;	 
...
//	Eth_send((u8 *)sfname);
	...
	{ 
		...
		sprintf((char *)eth,"{\"runtime\":%d.%1dms}\r\n",usmart_dev.runtime/10,usmart_dev.runtime%10);//打印函数执行时间 
		Eth_send((u8 *)eth);
	}	
}
//usmart扫描函数
...
void usmart_scan3(void)
{
	u8 sta,len;  
	if(USART3_RX_STA&0x8000)//串口接收完成？
	{					   
		len=USART3_RX_STA&0x3fff;	//得到此次接收到的数据长度
		USART3_RX_BUF[len]='\0';	//在末尾加入结束符. 
		sta=usmart_dev.cmd_rec(USART3_RX_BUF);//得到函数各个信息
		if(sta==0)usmart_dev.exe();	//执行函数 
		else 
		{  
			len=usmart_sys_cmd_exe(USART3_RX_BUF);
			if(len!=USMART_FUNCERR)sta=len;
			if(sta)
			{   
				switch(sta)
...
void Eth_usmart_scan(void)
{
	u8 sta,len;  
//	if(tcp_client_flag&1<<6)//是否收到数据?
//	{		
		//tcp_client_recvbuf		
		len=strlen((char *)tcp_client_recvbuf);	//得到此次接收到的数据长度
		tcp_client_recvbuf[len]='\0';	//在末尾加入结束符. 
		sta=usmart_dev.cmd_rec(tcp_client_recvbuf);//得到函数各个信息
		if(sta==0)usmart_dev.exe();	//执行函数 
		else 
		{  
			len=usmart_sys_cmd_exe(tcp_client_recvbuf);
			if(len!=USMART_FUNCERR)sta=len;
			if(sta)
			{   
				switch(sta)
				{
					...
					case USMART_PARMOVER:
						...
						break;		
					case USMART_NOFUNCFIND:
						Eth_send((u8*)"未找到匹配的函数!\r\n");   			
						break;		
				}
			}
		}
		tcp_client_flag &= ~LWIP_SEND_DATA;//标记数据已经被处理了.
//	}
}
